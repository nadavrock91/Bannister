...
public class LearningPage : ContentPage
{
    ...
    private int _currentVideoPage = 0;
    private int _totalVideoPages = 0;
    private List<LearningVideo> _filteredVideos = new();
    ...
    private async Task EnsureLearningGameExistsAsync()
    {
        ...
        var learningGame = games.FirstOrDefault(g => 
            string.Equals(g.DisplayName, "Learning", StringComparison.OrdinalIgnoreCase));
    ...
    private void BuildUI()
    {
        ...
        Grid.SetColumn(_prevPageBtn, 0);
        paginationRow.Children.Add(_prevPageBtn);
        ...
        Grid.SetColumn(_pageInfoLabel, 1);
        paginationRow.Children.Add(_pageInfoLabel);
        ...
        Grid.SetColumn(_nextPageBtn, 2);
        paginationRow.Children.Add(_nextPageBtn);

        videosContainer.Children.Add(paginationRow);
    ...
    /// <summary>
    /// Load videos with pagination - only renders current page
    /// </summary>
    private async Task LoadVideosPageAsync()
    {
        ...
        _currentFocus = GetVideoFocusSettings();
        
        // Update all pickers (only on page 0 to avoid flicker)
        ...
        _creatorMonthlyCounts = new Dictionary<string, int>();
        
        if (_currentFocus.IsActive)
        {
            _creatorMonthlyCounts = allVideos
                .Where(v => v.Status == "Completed" && 
                           v.CompletedAt >= monthStart &&
                           (v.Category ?? "Unsorted") == _currentFocus.Category)
        ...
        if (_selectedStatus == "Locked")
        {
            ...
            if (_currentFocus.IsActive && _selectedCategory == _currentFocus.Category)
            {
                videos = videos.Where(v => 
                    v.Status == "NotStarted" && 
                    _lockedCreators.Contains(v.Creator ?? "Unknown"));
            }
            else
            {
        ...
        _filteredVideos = videos.ToList();
        
        // Calculate pagination
        _totalVideoPages = (_filteredVideos.Count + PageSize - 1) / PageSize;
        if (_totalVideoPages == 0) _totalVideoPages = 1;
        if (_currentVideoPage >= _totalVideoPages) _currentVideoPage = _totalVideoPages - 1;
        ...
        // Show locked count header if viewing focus category (only on first page)
        if (_currentVideoPage == 0 && _currentFocus.IsActive && _selectedCategory == _currentFocus.Category && _lockedCreators.Count > 0 && _selectedStatus != "Locked")
        {
        ...
        var pageVideos = _filteredVideos
            .Skip(_currentVideoPage * PageSize)
            .Take(PageSize)
            .ToList();

        foreach (var video in pageVideos)
        {
            int creatorCount = _creatorMonthlyCounts.GetValueOrDefault(video.Creator ?? "Unknown", 0);
            bool isLocked = _lockedCreators.Contains(video.Creator ?? "Unknown");
            var card = BuildVideoCard(video, creatorCount, _currentFocus.CreatorMonthlyLimit, isLocked);
    ...
    {
        _prevPageBtn.IsEnabled = _currentVideoPage > 0;
        _nextPageBtn.IsEnabled = _currentVideoPage < _totalVideoPages - 1;
        
        int totalCount = _filteredVideos.Count;
        int startItem = _currentVideoPage * PageSize + 1;
        int endItem = Math.Min((_currentVideoPage + 1) * PageSize, totalCount);
        
        _pageInfoLabel.Text = totalCount > 0 
    ...
    {
        if (_currentVideoPage < _totalVideoPages - 1)
        {
            _currentVideoPage++;
            await LoadVideosPageAsync();
        }
    ...
    private Frame BuildVideoCard(LearningVideo video, int creatorMonthlyCount = 0, int creatorMonthlyLimit = 10, bool isCreatorLocked = false)
    {
        ...
        // Row 1: Watch + Transcript + Watched + Pending buttons
        var row1 = new HorizontalStackLayout { Spacing = 4 };
        
        // Watch button
        var watchBtn = CreateHoverButton("▶", "#2196F3", "#1976D2", 12, "Watch Video");
        ...
        {
            var transcriptBtn = CreateHoverButton("📝", "#FF9800", "#F57C00", 11, "Get Transcript");
        ...
        {
            var watchedBtn = CreateHoverButton("✓", "#4CAF50", "#388E3C", 14, "Mark as Watched");
        ...
        // Mark as pending button (only if not already pending or watched)
        ...
        // Category button
        var categoryBtn = CreateHoverButton("📁", "#607D8B", "#455A64", 11, "Change Category");
        ...
        {
            var readSummaryBtn = CreateHoverButton("📖", "#2196F3", "#1976D2", 11, "Mark as Read Summary");
        ...
        // Delete button
        var deleteBtn = CreateHoverButton("🗑", "#F44336", "#D32F2F", 11, "Delete Video");
        ...
        // More menu button
        var menuBtn = CreateHoverButton("⋮", "#9E9E9E", "#757575", 14, "More Options");
    ...
    /// </summary>
    /// <summary>
    /// Create a button with hover effect and tooltip for desktop
    /// </summary>
    private Button CreateHoverButton(string text, string normalColor, string hoverColor, int fontSize, string? tooltip = null)
    {
    ...
    private async void OnAddVideoClicked(object sender, EventArgs e)
    {
        ...
        // If fetch failed (generic title or channel), prompt user for info
        bool needsManualInput = 
            string.IsNullOrEmpty(title) || title == "Video" || title == "Unknown Title" ||
            string.IsNullOrEmpty(channelName) || channelName == "YouTube" || channelName == "Unknown Channel";
        
        if (needsManualInput)
        {
            var existingVideos = await _learning.GetVideosAsync(_auth.CurrentUsername);
            var existingChannels = existingVideos
                .Select(v => v.Creator)
                .Where(c => !string.IsNullOrEmpty(c) && c != "YouTube" && c != "Unknown Channel")
                .Distinct()
                .OrderBy(c => c)
                .ToList();
            
            (title, channelName) = await PromptForVideoInfoAsync(title, channelName, existingChannels);
        }
        ...
        // If no channel mapping exists, show categorization flow
    ...
    /// <summary>
    /// Show categorization flow for new video when no channel mapping exists
    /// Returns category name or null if cancelled
    /// </summary>
    private async Task<string?> ShowNewVideoCategorizeFlowAsync(string channelName)
    {
        var tcs = new TaskCompletionSource<string?>();
        
        var categories = GetCategories().Where(c => c != "Unsorted").OrderBy(c => c).ToList();
        ...
        contentStack.Children.Add(new Label
        {
            Text = $"No mapping exists for channel:\n👤 {channelName}",
            FontSize = 14,
            TextColor = Color.FromArgb("#666"),
            HorizontalOptions = LayoutOptions.Center,
            HorizontalTextAlignment = TextAlignment.Center
        ...
        // Option 1: Choose existing category
        contentStack.Children.Add(new Label
        {
        ...
        contentStack.Children.Add(categoryPicker);
        
        var btnUseSelected = new Button
        {
        ...
        {
            btnUseSelected.IsEnabled = categoryPicker.SelectedIndex >= 0;
        };
        contentStack.Children.Add(btnUseSelected);
        ...
        contentStack.Children.Add(newCategoryEntry);
        
        var btnCreateNew = new Button
        {
            Text = "Create & Use New Category",
            BackgroundColor = Color.FromArgb("#4CAF50"),
            TextColor = Colors.White,
            CornerRadius = 8,
            HeightRequest = 44,
        ...
        newCategoryEntry.TextChanged += (s, e) =>
        {
            btnCreateNew.IsEnabled = !string.IsNullOrWhiteSpace(newCategoryEntry.Text);
        };
        contentStack.Children.Add(btnCreateNew);
        ...
        // Option 3: Create channel mapping
        contentStack.Children.Add(new Label
        {
            Text = "Or Map This Channel:",
            FontSize = 14,
            FontAttributes = FontAttributes.Bold,
            TextColor = Color.FromArgb("#333")
        });
        
        var btnMapChannel = new Button
        {
            Text = $"🔗 Create Channel Mapping for '{channelName}'",
            BackgroundColor = Color.FromArgb("#FF9800"),
            TextColor = Colors.White,
            CornerRadius = 8,
            HeightRequest = 44,
        ...
        contentStack.Children.Add(btnMapChannel);
        ...
        bottomRow.Children.Add(btnUnsorted);
        ...
        // Wire up buttons
        btnUseSelected.Clicked += async (s, e) =>
        {
            var selectedCategory = categoryPicker.Items[categoryPicker.SelectedIndex];
            await Navigation.PopModalAsync(false);
            tcs.TrySetResult(selectedCategory);
        };
        
        btnCreateNew.Clicked += async (s, e) =>
        {
            var newCategory = newCategoryEntry.Text.Trim();
            ...
            await Navigation.PopModalAsync(false);
            tcs.TrySetResult(newCategory);
        };
        
        btnMapChannel.Clicked += async (s, e) =>
        {
            await Navigation.PopModalAsync(false);
            // Show channel mapping flow
            var mappedCategory = await ShowChannelMappingFlowAsync(channelName);
            tcs.TrySetResult(mappedCategory);
        };
        
        btnUnsorted.Clicked += async (s, e) =>
        {
            await Navigation.PopModalAsync(false);
            tcs.TrySetResult("Unsorted");
        };
        ...
        var popup = new Frame
        {
            ...
            HorizontalOptions = LayoutOptions.Center,
            VerticalOptions = LayoutOptions.Center,
            Content = new ScrollView { Content = contentStack, MaximumHeightRequest = 600 }
        };
    ...
    /// <summary>
    /// Show channel mapping flow - map channel to category, then ask whether to use it for this video
    /// </summary>
    private async Task<string?> ShowChannelMappingFlowAsync(string channelName)
    {
        var tcs = new TaskCompletionSource<string?>();
        ...
        contentStack.Children.Add(new Label
        {
            Text = "All future videos from this channel will be auto-categorized.",
            FontSize = 12,
            TextColor = Color.FromArgb("#999"),
            HorizontalOptions = LayoutOptions.Center,
            HorizontalTextAlignment = TextAlignment.Center
        ...
        var btnMapToExisting = new Button
        {
            Text = "Map to Selected",
            BackgroundColor = Color.FromArgb("#5B63EE"),
            TextColor = Colors.White,
            CornerRadius = 8,
            HeightRequest = 44,
        ...
        categoryPicker.SelectedIndexChanged += (s, e) =>
        {
            btnMapToExisting.IsEnabled = categoryPicker.SelectedIndex >= 0;
        };
        contentStack.Children.Add(btnMapToExisting);
        ...
        contentStack.Children.Add(new Label
        {
            Text = "Or Create New Category & Map:",
            FontSize = 14,
            FontAttributes = FontAttributes.Bold,
            TextColor = Color.FromArgb("#333")
        });
        ...
        newCategoryEntry.TextChanged += (s, e) =>
        {
            btnMapToNew.IsEnabled = !string.IsNullOrWhiteSpace(newCategoryEntry.Text);
        };
        contentStack.Children.Add(btnMapToNew);
        ...
        contentStack.Children.Add(btnCancel);
        
        // Wire up buttons
        btnMapToExisting.Clicked += async (s, e) =>
        {
            var selectedCategory = categoryPicker.Items[categoryPicker.SelectedIndex];
            // Save channel mapping
            var mappings = GetChannelMappings();
            mappings[channelName] = selectedCategory;
            SaveChannelMappings(mappings);
            
            await Navigation.PopModalAsync(false);
            
            // Ask whether to use mapping or choose different category for this video
            var useMapping = await AskUseChannelMappingAsync(channelName, selectedCategory);
            tcs.TrySetResult(useMapping);
        };
        
        btnMapToNew.Clicked += async (s, e) =>
        {
            var newCategory = newCategoryEntry.Text.Trim();
            // Add to categories list
            var cats = GetCategories();
            ...
            // Save channel mapping
            var mappings = GetChannelMappings();
            mappings[channelName] = newCategory;
            SaveChannelMappings(mappings);
            
            await Navigation.PopModalAsync(false);
            
            // Ask whether to use mapping or choose different category for this video
            var useMapping = await AskUseChannelMappingAsync(channelName, newCategory);
            tcs.TrySetResult(useMapping);
        };
        ...
        {
            await Navigation.PopModalAsync(false);
            // Go back to the first categorization dialog
            var category = await ShowNewVideoCategorizeFlowAsync(channelName);
        ...
        var popup = new Frame
        {
            ...
            HorizontalOptions = LayoutOptions.Center,
            VerticalOptions = LayoutOptions.Center,
            Content = new ScrollView { Content = contentStack, MaximumHeightRequest = 500 }
        };
    ...
    /// <summary>
    /// After creating channel mapping, ask whether to use it for this video or choose different category
    /// </summary>
    private async Task<string?> AskUseChannelMappingAsync(string channelName, string mappedCategory)
    {
        var tcs = new TaskCompletionSource<string?>();
        ...
        contentStack.Children.Add(new Label
        {
            Text = $"'{channelName}' → '{mappedCategory}'",
            FontSize = 14,
            TextColor = Color.FromArgb("#666"),
            HorizontalOptions = LayoutOptions.Center,
            HorizontalTextAlignment = TextAlignment.Center
        ...
        contentStack.Children.Add(new Label
        {
            Text = "For THIS video, do you want to:",
            FontSize = 14,
            FontAttributes = FontAttributes.Bold,
            TextColor = Color.FromArgb("#333"),
            HorizontalOptions = LayoutOptions.Center
        ...
        // Use the mapping
        var btnUseMapping = new Button
        {
            Text = $"✓ Use '{mappedCategory}'",
            BackgroundColor = Color.FromArgb("#4CAF50"),
            TextColor = Colors.White,
            CornerRadius = 8,
            HeightRequest = 48,
        ...
        contentStack.Children.Add(btnUseMapping);
        ...
        var btnUseDifferent = new Button
        {
        ...
        categoryPicker.SelectedIndexChanged += (s, e) =>
        {
            btnUseDifferent.IsEnabled = categoryPicker.SelectedIndex >= 0;
        };
        contentStack.Children.Add(btnUseDifferent);
        
        // Wire up buttons
        btnUseMapping.Clicked += async (s, e) =>
        {
            await Navigation.PopModalAsync(false);
            tcs.TrySetResult(mappedCategory);
        };
        
        btnUseDifferent.Clicked += async (s, e) =>
        {
            var selectedCategory = categoryPicker.Items[categoryPicker.SelectedIndex];
            await Navigation.PopModalAsync(false);
            tcs.TrySetResult(selectedCategory);
    ...
    private async Task ProcessImportAsync(string input)
    {
        ...
        // Get existing channels for fallback selection
        var existingVideos = await _learning.GetVideosAsync(_auth.CurrentUsername);
        var existingChannels = existingVideos
            .Select(v => v.Creator)
            .Where(c => !string.IsNullOrEmpty(c) && c != "YouTube" && c != "Unknown Channel")
        ...
        foreach (var videoId in videoIds)
        {
            ...
            if (needsManualInput)
            {
                ...
                // Add to existing channels if it's a new one
                if (!string.IsNullOrEmpty(channelName) && 
                    channelName != "YouTube" && 
                    !existingChannels.Contains(channelName))
                {
                    existingChannels.Add(channelName);
                    existingChannels = existingChannels.OrderBy(c => c).ToList();
                }
            }
    ...
    /// <summary>
    /// Prompt user to select a channel when auto-fetch fails
    /// </summary>
    private async Task<string> PromptForChannelSelectionAsync(string videoTitle, List<string> existingChannels)
    {
        // Build options list
        var options = new List<string>();
        ...
        options.Add("➕ Enter New Channel");
        options.Add("⏭️ Skip (leave as YouTube)");
        
        string displayTitle = videoTitle.Length > 50 ? videoTitle.Substring(0, 47) + "..." : videoTitle;
        ...
        if (result == null || result == "⏭️ Skip (leave as YouTube)")
        {
            return "YouTube";
        }
        else if (result == "➕ Enter New Channel")
        {
            string newChannel = await DisplayPromptAsync("New Channel", "Enter channel name:");
            return string.IsNullOrWhiteSpace(newChannel) ? "YouTube" : newChannel.Trim();
        }
        else
        {
            return result;
    ...
    /// <summary>
    /// Prompt user for both title and channel when auto-fetch fails
    /// </summary>
    private async Task<(string title, string channel)> PromptForVideoInfoAsync(string defaultTitle, string defaultChannel, List<string> existingChannels)
    {
        string title = defaultTitle;
        string channel = defaultChannel;
        
        // If title is generic, ask user to enter it
        if (string.IsNullOrEmpty(title) || title == "Video" || title == "Unknown Title")
        {
            string? enteredTitle = await DisplayPromptAsync(
                "Video Title",
                "Could not fetch title. Please enter it:",
        ...
        // If channel is generic, ask user to select/enter it
        if (string.IsNullOrEmpty(channel) || channel == "YouTube" || channel == "Unknown Channel")
        {
            channel = await PromptForChannelSelectionAsync(title, existingChannels);
        }
    ...
    private async Task CategorizeVideosSequentiallyAsync(List<LearningVideo> videos)
    {
        ...
        foreach (var video in videos)
        {
            ...
            // Use custom modal to show full title
            var tcs = new TaskCompletionSource<string?>();
            ...
            contentStack.Children.Add(new Label
            {
                Text = $"Categorize Videos ({current}/{total})",
                FontSize = 16,
                FontAttributes = FontAttributes.Bold,
                TextColor = Color.FromArgb("#333"),
                HorizontalOptions = LayoutOptions.Center
            ...
            // Full video title - wrapped
            contentStack.Children.Add(new Frame
            {
                Padding = 12,
                CornerRadius = 8,
                BackgroundColor = Color.FromArgb("#F5F5F5"),
            ...
            // Category buttons in a scrollable area
            var categoryScroll = new ScrollView { HeightRequest = 250 };
            var categoryStack = new VerticalStackLayout { Spacing = 6 };
            
            foreach (var option in options)
            {
                bool isAction = option.StartsWith("➕") || option.StartsWith("⏭️") || option.StartsWith("⏹️");
                
                var btn = new Button
                {
                    Text = option,
                    BackgroundColor = isAction 
                        ? (option.StartsWith("⏹️") ? Color.FromArgb("#F44336") 
                           : option.StartsWith("⏭️") ? Color.FromArgb("#9E9E9E")
                ...
                btn.Clicked += (s, e) => tcs.TrySetResult(option);
                categoryStack.Children.Add(btn);
            }
            
            categoryScroll.Content = categoryStack;
            contentStack.Children.Add(categoryScroll);
