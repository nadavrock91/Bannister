...
public partial class EditActivityPage : ContentPage
{
    ...
    protected override async void OnAppearing()
    {
        ...
        _previousCategoryIndex = pickerCategory.SelectedIndex;
        
        // Subscribe to selection change to handle "New Category"
        pickerCategory.SelectedIndexChanged += OnCategoryPickerChanged;
    ...
    {
        if (pickerCategory.SelectedIndex < 0) return;
        ...
        if (selected == "➕ New Category...")
        {
            ...
            if (!string.IsNullOrWhiteSpace(newCategory))
            {
                ...
                int insertIndex = _categoryOptions.Count - 1;
                _categoryOptions.Insert(insertIndex, newCategory);
                
                // Refresh picker
                pickerCategory.ItemsSource = null;
                pickerCategory.ItemsSource = _categoryOptions;
                
                // Select the new category
                pickerCategory.SelectedIndex = insertIndex;
                _previousCategoryIndex = insertIndex;
            }
            else
            {
                // User cancelled - revert to previous selection
                pickerCategory.SelectedIndex = _previousCategoryIndex;
            }
        }
        else
        {
