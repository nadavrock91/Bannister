...
/// <summary>
/// Shows EXP over time chart for each game in a grid of cards
/// </summary>
public class ExpOverTimeChartsPage : ContentPage
{
    ...
    public ExpOverTimeChartsPage(AuthService auth, GameService games, DatabaseService db)
    {
        _auth = auth;
        _games = games;
        _db = db;
    ...
    private void BuildUI()
    {
        ...
        mainStack.Children.Add(new Label
        {
            Text = "Total EXP progression for each game",
            FontSize = 14,
    ...
    private async Task<Frame> CreateGameChartCardAsync(Game game)
    {
        ...
        {
            // Calculate cumulative EXP data points
            var chartData = CalculateExpChartData(logs);
            
            // Total EXP
            int totalExp = chartData.Count > 0 ? chartData.Last().Value : 0;

            stack.Children.Add(new Label
            {
                Text = $"{totalExp:N0} EXP",
                FontSize = 14,
                TextColor = Color.FromArgb("#5B63EE"),
                FontAttributes = FontAttributes.Bold
            });

            // Chart - takes most of the space (using compact drawable)
            var drawable = new ExpChartCompactDrawable(chartData);
    ...
    private List<ChartDataPoint> CalculateExpChartData(List<ExpLog> logs)
    {
        if (logs.Count == 0)
            return new List<ChartDataPoint>();

        // Group by day and calculate cumulative EXP
        var dailyData = logs
            .GroupBy(l => l.LoggedAt.Date)
            .OrderBy(g => g.Key)
            .Select(g => new { Date = g.Key, DailyExp = g.Sum(l => l.DeltaExp) })
            .ToList();
        ...
        {
            cumulativeExp += day.DailyExp;
            result.Add(new ChartDataPoint { Date = day.Date, Value = cumulativeExp });
        }
