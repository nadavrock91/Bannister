...
{
    private readonly AuthService _auth; private readonly GameService _gameService; private readonly DatabaseService _db; private readonly LifePathService? _lifePathService; private readonly PrivacyModeService _privacyMode;
    ...
    public LifePathsPage(AuthService auth, GameService gameService, DatabaseService db, LifePathService? lifePathService=null, PrivacyModeService? privacyMode=null){_auth=auth;_gameService=gameService;_db=db;_lifePathService=lifePathService;_privacyMode=privacyMode??new PrivacyModeService();Title="Life Paths";BackgroundColor=Color.FromArgb("#0D1117");BuildUI();}
    ...
    private void RenderRows(){_rowsContainer.Children.Clear();_axisContainer.Children.Clear();if(_gameRows.Count==0)return;var(rStart,rEnd,span)=Range();float barW=GetBarW();_axisContainer.Children.Add(BuildAxis(rStart,rEnd,span,barW));var displayMode=_privacyMode.GetDisplayMode(_auth.CurrentUsername);var modeFiltered=_gameRows.Where(g=>displayMode switch{ActivityDisplayMode.PublicOnly=>g.GameVisibility==1||g.GameVisibility==2,ActivityDisplayMode.PrivateOnly=>g.GameVisibility==0||g.GameVisibility==2,_=>true}).ToList();var sortedGames=modeFiltered.OrderBy(g=>g.DisplayName,StringComparer.OrdinalIgnoreCase).ToList();List<GameTimelineRow> visibleGames;if(_isExpanded){visibleGames=sortedGames;}else if(_pinnedGameIds.Count>0){visibleGames=sortedGames.Where(g=>_pinnedGameIds.Contains(g.GameId)).ToList();}else{_rowsContainer.Children.Add(new Label{Text="No games pinned yet.\nTap ⊞ All to see all games, then tap · next to each game you want to monitor here.",FontSize=13,TextColor=Color.FromArgb("#8B949E"),LineBreakMode=LineBreakMode.WordWrap,Margin=new Thickness(0,8)});return;}foreach(var g in visibleGames)_rowsContainer.Children.Add(BuildGameRow(g,rStart,span,barW));}
    ...
    internal class GameTimelineRow{public string GameId{get;set;}="";public string DisplayName{get;set;}="";public int GameVisibility{get;set;}=1;public DateTime StartDate{get;set;}public DateTime EndDate{get;set;}public DateTime? EndedAt{get;set;}public bool IsActive{get;set;}public int TotalLogs{get;set;}public List<ActivePeriod> Periods{get;set;}=new();public List<SubBlockData> SubBlocks{get;set;}=new();}
