...
public class LifePathsPage : ContentPage
{
    ...
    private void RenderRows(){_rowsContainer.Children.Clear();_axisContainer.Children.Clear();if(_gameRows.Count==0)return;var(rStart,rEnd,span)=Range();float barW=GetBarW();_axisContainer.Children.Add(BuildAxis(rStart,rEnd,span,barW));var sortedGames=_gameRows.OrderBy(g=>g.DisplayName,StringComparer.OrdinalIgnoreCase).ToList();var visibleGames=!_isExpanded&&_pinnedGameIds.Count>0?sortedGames.Where(g=>_pinnedGameIds.Contains(g.GameId)).ToList():sortedGames;foreach(var g in visibleGames)_rowsContainer.Children.Add(BuildGameRow(g,rStart,span,barW));}
