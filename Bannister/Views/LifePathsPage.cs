...
public class TimelineBarDrawable : IDrawable
{
    ...
    public TimelineBarDrawable(DateTime start, double span, float width, float rowH, float barH, Color color, IReadOnlyList<PeriodData> periods, IReadOnlyList<SubBlockData> blocks, DateTime? endedAt, bool isActive)
    { _rangeStart=start; _span=span; _barW=width; _rowH=rowH; _barH=barH; _base=color; _periods=periods; _blocks=blocks; _endedAt=endedAt; }
...
public class LifePathsPage : ContentPage
{
    ...
    private List<GameTimelineRow> _gameRows=new(); private List<Game> _allGames=new(); private string _zoom="year";
    ...
    private float GetBarW()
    {
        ...
        float available=(float)screenW-LabelW-EditW-Gutter*3-32;
    ...
    private async Task LoadDataAsync(){_statusLabel.Text="Loading...";_generateBtn.IsEnabled=false;try{var(rows,games)=await BuildTimelineDataAsync();_gameRows=rows;_allGames=games;_statusLabel.Text=$"{rows.Count} games · {rows.Sum(r=>r.TotalLogs):N0} records";RenderRows();}catch(Exception ex){_statusLabel.Text=$"Error: {ex.Message}";}finally{_generateBtn.IsEnabled=true;}}
    ...
    private void RenderRows(){_rowsContainer.Children.Clear();_axisContainer.Children.Clear();if(_gameRows.Count==0)return;var(rStart,rEnd,span)=Range();float barW=GetBarW();_axisContainer.Children.Add(BuildAxis(rStart,rEnd,span,barW));foreach(var g in _gameRows.OrderBy(g=>g.StartDate))_rowsContainer.Children.Add(BuildGameRow(g,rStart,span,barW));}
    private View BuildAxis(DateTime start,DateTime end,double span,float barW){float edit=_lifePathService!=null?EditW+Gutter:0;var grid=new Grid{ColumnDefinitions={new ColumnDefinition(new GridLength(LabelW)),new ColumnDefinition(new GridLength(barW)),new ColumnDefinition(new GridLength(edit))}};grid.Add(new Label{Text=""},0,0);grid.Add(new GraphicsView{Drawable=new AxisDrawable(start,end,span,barW),HeightRequest=20,WidthRequest=barW},1,0);return grid;}
    private View BuildGameRow(GameTimelineRow game,DateTime start,double span,float barW)
    {
        bool level2=game.SubBlocks.Any(b=>b.Level==2);float rowH=level2?42:game.SubBlocks.Count>0?30:20;int hue=Math.Abs(game.GameId.GetHashCode())%360;var color=Color.FromHsv(hue/360f,.65f,.78f);var drawable=new TimelineBarDrawable(start,span,barW,rowH,BarH,color,game.Periods.Select(p=>new PeriodData(p.Start,p.End,p.IsGap,p.LogCount)).ToList(),game.SubBlocks,game.EndedAt,game.IsActive);var canvas=new GraphicsView{Drawable=drawable,HeightRequest=rowH,WidthRequest=barW};var tap=new TapGestureRecognizer();tap.Tapped+=async(_,_)=>await ShowDetailAsync(game);canvas.GestureRecognizers.Add(tap);float edit=_lifePathService!=null?EditW+Gutter:0;var row=new Grid{ColumnDefinitions={new ColumnDefinition(new GridLength(LabelW)),new ColumnDefinition(new GridLength(barW)),new ColumnDefinition(new GridLength(edit))},ColumnSpacing=Gutter};row.Add(new Label{Text=game.DisplayName,FontSize=10,TextColor=game.IsActive?Color.FromArgb("#C9D1D9"):Color.FromArgb("#484F58"),LineBreakMode=LineBreakMode.TailTruncation,MaxLines=2},0,0);row.Add(canvas,1,0);if(_lifePathService!=null){var btn=new Button{Text="✏",HeightRequest=20,WidthRequest=EditW,Padding=0,BackgroundColor=Color.FromArgb("#21262D"),TextColor=Color.FromArgb("#8B949E")};var captured=_allGames.FirstOrDefault(g=>g.GameId==game.GameId);btn.Clicked+=async(_,_)=>{if(captured==null)return;await Navigation.PushAsync(new LifePathEditorPage(captured,_lifePathService!,_gameService,_auth));await LoadDataAsync();};row.Add(btn,2,0);}return row;
    }
