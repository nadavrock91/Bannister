...
public class LifePathsPage : ContentPage
{
    ...
    private void BuildUI()
    {
        ...
        var controls=new HorizontalStackLayout{Spacing=6};_btnMonth=Zoom("Month",false);_btnYear=Zoom("Year",true);_btnDecade=Zoom("Decade",false);_btnMonth.Clicked+=(_,_)=>SetZoom("month");_btnYear.Clicked+=(_,_)=>SetZoom("year");_btnDecade.Clicked+=(_,_)=>SetZoom("decade");controls.Children.Add(_btnMonth);controls.Children.Add(_btnYear);controls.Children.Add(_btnDecade);_generateBtn=new Button{Text="",HeightRequest=28,WidthRequest=36,Padding=0,BackgroundColor=Color.FromArgb("#21262D"),TextColor=Color.FromArgb("#58A6FF")};_generateBtn.Clicked+=async(_,_)=>await LoadDataAsync();controls.Children.Add(_generateBtn);_expandBtn=new Button{Text="⊞ Expand",FontSize=11,HeightRequest=28,CornerRadius=4,Padding=new Thickness(10,0),BackgroundColor=Color.FromArgb("#21262D"),TextColor=Color.FromArgb("#8B949E"),BorderColor=Color.FromArgb("#30363D"),BorderWidth=1};_expandBtn.Clicked+=(_,_)=>{_isExpanded=!_isExpanded;_expandBtn.Text=_isExpanded?"⊟ Collapse":"⊞ Expand";_expandBtn.TextColor=_isExpanded?Colors.White:Color.FromArgb("#8B949E");RenderRows();};controls.Children.Add(_expandBtn);page.Children.Add(controls);
    ...
    private void RenderRows(){_rowsContainer.Children.Clear();_axisContainer.Children.Clear();if(_gameRows.Count==0)return;var(rStart,rEnd,span)=Range();float barW=GetBarW();_axisContainer.Children.Add(BuildAxis(rStart,rEnd,span,barW));foreach(var g in _gameRows.OrderBy(g=>g.DisplayName,StringComparer.OrdinalIgnoreCase))_rowsContainer.Children.Add(BuildGameRow(g,rStart,span,barW));}
