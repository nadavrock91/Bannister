...
public partial class ActivityGamePage
{
    ...
    private async Task RefreshActivitiesAsync()
    {
        ...
        if (_currentMetaFilter != "Possible" && _currentMetaFilter != "Expired" && _currentMetaFilter != "Stale" && _currentMetaFilter != "Missing Image")
        {
            ...
            {
                // Viewing a non-navigable category via dropdown
                categoryToFilter = _tempNonNavigableCategory;
            }
            else if (_navigableCategories.Count > 0 && _currentCategoryIndex >= 0 && _currentCategoryIndex < _navigableCategories.Count)
            {
                // Normal navigable category
                categoryToFilter = _navigableCategories[_currentCategoryIndex];
            }
            
            if (categoryToFilter != null)
            {
                filtered = filtered.Where(vm => 
                    string.Equals(vm.Category, categoryToFilter, StringComparison.OrdinalIgnoreCase)).ToList();
            }
        }
    ...
    {
        // Handle viewing a non-navigable category (selected via dropdown)
        if (_tempNonNavigableCategory != null)
        {
            ...
            int tempPickerIndex = _categories.FindIndex(c => 
        ...
        int pickerIndex = _categories.FindIndex(c => 
    ...
    private async Task CheckBrokenStreaksAsync()
    {
        ...
        if (brokenStreaks.Count > 0)
        {
            ...
            var streakPage = new StreakBrokenPage(_auth, _activities, _exp, _streaks, _game.GameId, brokenStreaks);
