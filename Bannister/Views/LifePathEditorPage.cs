...
public class LifePathEditorPage : ContentPage
{
    ...
    private void BuildTimelineRows(float labelW, DateTime gameStart, DateTime gameEnd)
    {
        ...
        for (int i = 0; i < level1.Count; i++) { var b = level1[i]; var color = blockColors[b.Id]; int bDays = (int)((b.EndDate ?? DateTime.Now) - b.StartDate).TotalDays; string bDur = bDays >= 365 ? $"{bDays / 365}y {bDays % 365}d" : $"{bDays}d"; string bLabel = $"{b.Label}  {bDur}"; float bH = ComputeRowHeight(bLabel, 200f, 12f, 28f); _timelineRows.Children.Add(BuildRow(bLabel, Colors.White, _selectedBlockId == b.Id, false, b.StartDate, b.EndDate, color, b.Id, bH)); foreach (var c in _blocks.Where(c => c.ParentId == b.Id).OrderBy(c => c.StartDate)) { int cDays = (int)((c.EndDate ?? DateTime.Now) - c.StartDate).TotalDays; string cDur = cDays >= 365 ? $"{cDays / 365}y {cDays % 365}d" : $"{cDays}d"; string cLabel = $"  ↳ {c.Label}  {cDur}"; float cH = ComputeRowHeight(cLabel, 186f, 11f, 24f); var childColor = blockColors.TryGetValue(c.Id, out var cc) ? cc : color; _timelineRows.Children.Add(BuildRow(cLabel, Color.FromArgb("#C9D1D9"), _selectedBlockId == c.Id, false, c.StartDate, c.EndDate, childColor, c.Id, cH)); } }
    }
