...
public class LifePathEditorPage : ContentPage
{
    ...
    {
        var drawable = new RowBarDrawable { RangeStart = _rangeStart, TotalSpanDays = _totalSpanDays, Width = _barW, Height = height, BarColor = color, IsGameRow = gameRow, IsSelected = selected, BlockStart = start, BlockEnd = end, Periods = _periods, FocusSegments = focusSegments ?? new() }; var canvas = new GraphicsView { Drawable = drawable, HeightRequest = height, WidthRequest = _barW }; if (blockId.HasValue) { var id = blockId.Value; var tap = new TapGestureRecognizer(); tap.Tapped += (_, _) => { _selectedBlockId = _selectedBlockId == id ? null : id; BuildTimelineRows(200, _game.CreatedAt.ToLocalTime(), _game.LifePathEndedAt?.ToLocalTime() ?? DateTime.Now); RenderDetailPanel(); }; canvas.GestureRecognizers.Add(tap); } var row = new Grid { ColumnDefinitions = { new ColumnDefinition(new GridLength(200)), new ColumnDefinition(new GridLength(_barW)) } }; row.Add(new Label { Text = label, FontSize = gameRow ? 12 : 12, FontAttributes = selected || gameRow ? FontAttributes.Bold : FontAttributes.None, TextColor = labelColor, Padding = new Thickness(16, 0, 4, 0), VerticalOptions = LayoutOptions.Center, LineBreakMode = LineBreakMode.WordWrap }, 0, 0); row.Add(canvas, 1, 0); return row;
    }
