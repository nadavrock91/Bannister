...
public class EditorAxisDrawable : IDrawable
{
    ...
    {
        _width = width; float X(DateTime d) => (float)((d - start).TotalDays / Math.Max(1d, span) * width); bool decade = span > 2000, year = span > 60;
...
public class LifePathEditorPage : ContentPage
{
    ...
    private void BuildTimelineRows(float labelW, DateTime gameStart, DateTime gameEnd)
    {
        ...
        var hue = Math.Abs(_game.GameId.GetHashCode()) % 360; var gameColor = Color.FromHsv(hue / 360f, .55f, .70f); _timelineRows.Children.Add(BuildRow(_game.DisplayName, Colors.White, true, true, gameStart, gameEnd, gameColor, null, 26));
        var level1 = _blocks.Where(b => b.ParentId == null).OrderBy(b => b.StartDate).ToList(); if (level1.Count == 0) { _timelineRows.Children.Add(new Label { Text = "  No focus blocks yet — tap + to add", FontSize = 12, TextColor = Color.FromArgb("#484F58"), FontAttributes = FontAttributes.Italic, Margin = new Thickness(16, 8) }); return; } _timelineRows.Children.Add(new BoxView { HeightRequest = 4, BackgroundColor = Color.FromArgb("#0D1117") });
        for (int i = 0; i < level1.Count; i++) { var b = level1[i]; var color = string.IsNullOrWhiteSpace(b.ColorHex) ? Color.FromArgb(Palette[i % Palette.Length]) : Color.FromArgb(b.ColorHex); _timelineRows.Children.Add(BuildRow(b.Label, Colors.White, _selectedBlockId == b.Id, false, b.StartDate, b.EndDate, color, b.Id, 24)); foreach (var c in _blocks.Where(c => c.ParentId == b.Id).OrderBy(c => c.StartDate)) _timelineRows.Children.Add(BuildRow("  ↳ " + c.Label, Color.FromArgb("#8B949E"), _selectedBlockId == c.Id, false, c.StartDate, c.EndDate, color.WithAlpha(.65f), c.Id, 20)); }
    }
    private View BuildRow(string label, Color labelColor, bool selected, bool gameRow, DateTime start, DateTime? end, Color color, int? blockId, float height)
    {
        var drawable = new RowBarDrawable { RangeStart = _rangeStart, TotalSpanDays = _totalSpanDays, Width = _barW, Height = height, BarColor = color, IsGameRow = gameRow, IsSelected = selected, BlockStart = start, BlockEnd = end, Periods = _periods }; var canvas = new GraphicsView { Drawable = drawable, HeightRequest = height, WidthRequest = _barW }; if (blockId.HasValue) { var id = blockId.Value; var tap = new TapGestureRecognizer(); tap.Tapped += (_, _) => { _selectedBlockId = _selectedBlockId == id ? null : id; BuildTimelineRows(120, _game.CreatedAt.ToLocalTime(), _game.LifePathEndedAt?.ToLocalTime() ?? DateTime.Now); RenderDetailPanel(); }; canvas.GestureRecognizers.Add(tap); } var row = new Grid { ColumnDefinitions = { new ColumnDefinition(new GridLength(120)), new ColumnDefinition(new GridLength(_barW)) } }; row.Add(new Label { Text = label, FontSize = gameRow ? 12 : 11, FontAttributes = selected || gameRow ? FontAttributes.Bold : FontAttributes.None, TextColor = labelColor, Padding = new Thickness(16, 0, 4, 0), VerticalOptions = LayoutOptions.Center, LineBreakMode = LineBreakMode.TailTruncation }, 0, 0); row.Add(canvas, 1, 0); return row;
    }
    private void RenderDetailPanel()
    { _detailPanel.Children.Clear(); if (_selectedBlockId == null) { _detailPanel.Children.Add(new Label { Text = "Tap a focus row to see details, edit, or add a sub-block.", FontSize = 12, TextColor = Color.FromArgb("#484F58"), FontAttributes = FontAttributes.Italic }); return; } var b = _blocks.FirstOrDefault(x => x.Id == _selectedBlockId); if (b == null) return; var inner = new VerticalStackLayout { Spacing = 6 }; inner.Children.Add(new Label { Text = b.Label, FontSize = 15, FontAttributes = FontAttributes.Bold, TextColor = Colors.White }); inner.Children.Add(new Label { Text = $"{b.StartDate:dd MMM yyyy} → {(b.EndDate.HasValue ? b.EndDate.Value.ToString("dd MMM yyyy") : "ongoing")}", FontSize = 12, TextColor = Color.FromArgb("#58A6FF") }); if (!string.IsNullOrWhiteSpace(b.Reason)) inner.Children.Add(new Label { Text = b.Reason, FontSize = 12, TextColor = Color.FromArgb("#8B949E") }); var edit = new Button { Text = "✏️ Edit", HeightRequest = 34, BackgroundColor = Color.FromArgb("#21262D"), TextColor = Colors.White }; edit.Clicked += async (_, _) => await EditBlockAsync(b); var buttons = new HorizontalStackLayout { Spacing = 8 }; buttons.Children.Add(edit); if (b.Level < 2) { var sub = new Button { Text = "+ Sub-block", HeightRequest = 34, BackgroundColor = Color.FromArgb("#21262D"), TextColor = Color.FromArgb("#58A6FF") }; sub.Clicked += async (_, _) => await AddBlockAsync(b.Id, b.Level + 1); buttons.Children.Add(sub); } var del = new Button { Text = "", HeightRequest = 34, WidthRequest = 40, BackgroundColor = Color.FromArgb("#21262D"), TextColor = Color.FromArgb("#F85149") }; del.Clicked += async (_, _) => { if (await DisplayAlert("Delete", $"Delete \"{b.Label}\" and sub-blocks?", "Delete", "Cancel")) { await _lifePathService.DeleteAsync(b.Id); _selectedBlockId = null; await LoadAsync(); } }; buttons.Children.Add(del); inner.Children.Add(buttons); _detailPanel.Children.Add(new Frame { Content = inner, Padding = 14, BackgroundColor = Color.FromArgb("#161B22"), BorderColor = Color.FromArgb("#30363D"), CornerRadius = 8, HasShadow = false }); }
