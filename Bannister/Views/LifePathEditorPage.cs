...
public class LifePathEditorPage : ContentPage
{
    ...
    private void BuildTimelineRows(float labelW, DateTime gameStart, DateTime gameEnd)
    {
        ...
        var hue = Math.Abs(_game.GameId.GetHashCode()) % 360; var gameColor = Color.FromHsv(hue / 360f, .55f, .70f); var level1 = _blocks.Where(b => b.ParentId == null).OrderBy(b => b.StartDate).ToList();
        ...
        _timelineRows.Children.Add(BuildRow(_game.DisplayName, Colors.White, true, true, gameStart, gameEnd, gameColor, null, 26, focusSegments));
        ...
        for (int i = 0; i < level1.Count; i++) { var b = level1[i]; var color = blockColors[b.Id]; int bDays = (int)((b.EndDate ?? DateTime.Now) - b.StartDate).TotalDays; string bDur = bDays >= 365 ? $"{bDays / 365}y {bDays % 365}d" : $"{bDays}d"; string bLabel = $"{b.Label}  {bDur}"; _timelineRows.Children.Add(BuildRow(bLabel, Colors.White, _selectedBlockId == b.Id, false, b.StartDate, b.EndDate, color, b.Id, 24)); foreach (var c in _blocks.Where(c => c.ParentId == b.Id).OrderBy(c => c.StartDate)) { int cDays = (int)((c.EndDate ?? DateTime.Now) - c.StartDate).TotalDays; string cDur = cDays >= 365 ? $"{cDays / 365}y {cDays % 365}d" : $"{cDays}d"; string cLabel = $"  ↳ {c.Label}  {cDur}"; _timelineRows.Children.Add(BuildRow(cLabel, Color.FromArgb("#8B949E"), _selectedBlockId == c.Id, false, c.StartDate, c.EndDate, color.WithAlpha(.65f), c.Id, 20)); } }
    }
    private View BuildRow(string label, Color labelColor, bool selected, bool gameRow, DateTime start, DateTime? end, Color color, int? blockId, float height, List<(DateTime, DateTime?, Color)>? focusSegments = null)
    {
