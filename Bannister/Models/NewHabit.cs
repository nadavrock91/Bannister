...
public class HabitAllowance
{
    ...
    // Countdown: how many more graduations needed before allowance can increase
    // Starts at CurrentAllowance, decrements on each graduation, prompts at 0
    // Reset to CurrentAllowance after increase or when manually edited
    public int GraduationsUntilIncrease { get; set; } = 1;
    
    // Weekly fill requirement tracking (for Daily habits)
    // The week start (Sunday) when slots were last filled to allowance
    public DateTime? LastFilledWeekStart { get; set; }
